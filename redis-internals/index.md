# Redis 내부 구조 — SDS, ziplist, skiplist

## 개요

Redis의 핵심 자료구조 구현을 소스 코드 레벨에서 분석하고 메모리 최적화 전략을 이해한다.
...
redis의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/redis/config
sudo redisctl status
sudo redisctl apply -f config.yaml
```
