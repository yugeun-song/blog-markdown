# async fn in traits — RPITIT와 AFIT 이해하기

## 개요

Rust 1.75의 async fn in traits 안정화 배경과 dyn 호환성 문제를 분석한다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/rust/config
sudo rustctl status
sudo rustctl apply -f config.yaml
```
