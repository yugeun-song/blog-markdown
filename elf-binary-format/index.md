...
ELF 헤더, 프로그램 헤더, 섹션 헤더를 readelf로 분석하고 동적 링킹 과정을 추적한다.
...
elf의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/elf/config
sudo elfctl status
sudo elfctl apply -f config.yaml
```
