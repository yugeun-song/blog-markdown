# HTTP/3과 QUIC — UDP 기반 전송 프로토콜의 혁신

## 개요

QUIC의 연결 마이그레이션, 스트림 다중화, 0-RTT 연결 재개를 프로토콜 레벨에서 분석한다.
...
http3의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/http3/config
sudo http3ctl status
sudo http3ctl apply -f config.yaml
```
