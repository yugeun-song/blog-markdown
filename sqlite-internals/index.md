# SQLite B-tree와 WAL 내부 구현

## 개요

SQLite의 페이지 기반 B-tree 구조와 WAL 저널 모드의 동시성 제어를 분석한다.
...
sqlite의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/sqlite/config
sudo sqlitectl status
sudo sqlitectl apply -f config.yaml
```
