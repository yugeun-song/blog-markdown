# Python 디스크립터 프로토콜 내부 동작

## 개요

__get__, __set__, __delete__의 호출 메커니즘과 property, classmethod 구현 원리를 분석한다.
...
python의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/python/config
sudo pythonctl status
sudo pythonctl apply -f config.yaml
```
