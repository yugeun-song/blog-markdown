# strace 고급 활용 — 시스템 콜 추적의 기술

## 개요

strace의 필터링, 통계, 인젝션 기능과 프로덕션 환경에서의 안전한 사용법을 정리한다.
...
strace의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/strace/config
sudo stracectl status
sudo stracectl apply -f config.yaml
```
