# Modern CMake 실전 가이드 — 타겟 기반 빌드

## 개요

CMake 3.x의 타겟 중심 설계와 INTERFACE/PUBLIC/PRIVATE 속성을 활용한 빌드 구성을 정리한다.
...
cmake의 기본적인 동작 원리를 이해하려면 먼저 내부 구조를 파악해야 한다. 아래 예제를 통해 핵심 메커니즘을 확인할 수 있다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/cmake/config
sudo cmakectl status
sudo cmakectl apply -f config.yaml
```
