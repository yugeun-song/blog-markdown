# Linux Capabilities — root 권한 분리의 실체

## 개요

POSIX capabilities 모델과 effective/permitted/inheritable 셋의 동작을 실험으로 확인한다.
...
```bash
# 환경 설정
export CONFIG_PATH=/etc/linux/config
sudo linuxctl status
sudo linuxctl apply -f config.yaml
```
